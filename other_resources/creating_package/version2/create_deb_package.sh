...
POSTINST="#!/bin/sh\nchmod 0755 /opt/shotbox/start.sh /opt/shotbox/etc-init.d/shotbox\nmv /opt/shotbox/etc-init.d/shotbox /etc/init.d/shotbox\nrm -rf /opt/shotbox/etc-init.d\nupdate-rc.d shotbox defaults\nservice shotbox start\n"
#POSTRM="#!/bin/sh\nrm -rfv /opt/shotbox/\nrm /tmp/shotbox-pid\nupdate-rc.d -f  shotbox remove\nrm /etc/init.d/shotbox\nmkdir -p /opt"
POSTRM="#!/bin/sh\nmkdir -p /opt"
...
mkdir ${PRODUCT}_${VERSION}/opt/shotbox/etc-init.d
cp system-v_service/shotbox ${PRODUCT}_${VERSION}/opt/shotbox/etc-init.d/
